...
export function DonationPaymentOptions() {
  ...
  async function startPaystackCheckout(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/payments/initiate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: form.get('email'), amountKes: form.get('amountKes') }),
    }).catch(() => null)

    if (!response) {
      setPaystackError('Could not connect to Paystack. Please try again or use another method.')
    ...
    if (!response.ok || !payload.authorizationUrl) {
      setPaystackError(payload.error ?? 'Paystack checkout could not be started.')
      setPaystackLoading(false)
      return
    }
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-3" data-testid="payment-options">
      ...
      <article className="flex flex-col rounded-2xl border border-navy-100 bg-white p-7 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-gold-600">
          Kenya and international
        </p>
        <h3 className="mt-3 font-heading text-2xl font-bold text-navy">
          Paystack
        </h3>
        <p className="mt-3 leading-7 text-navy-600">
          Pay securely in KES through Paystack using the payment methods available for your account.
        </p>
        <form className="mt-6 space-y-4" onSubmit={startPaystackCheckout}>
          <label className="block text-sm font-semibold text-navy" htmlFor="support-paystack-email">
            Email address
          </label>
          <input
            id="support-paystack-email"
            name="email"
            type="email"
            autoComplete="email"
            required
          ...
          />
          <label className="block text-sm font-semibold text-navy" htmlFor="support-amount-kes">
            Amount in KES
          </label>
          <input
            id="support-amount-kes"
            name="amountKes"
            type="number"
            inputMode="numeric"
            min="100"
          ...
          <Button type="submit" loading={paystackLoading} className="w-full">
